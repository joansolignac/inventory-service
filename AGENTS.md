...
domain in `modules/`: `category/`, `product/`, and `reservation/`. Within a module,
...
The `reservation/` module owns the stock-reservation workflow. Reservations use
a client-provided UUID so external services can retry safely. Keep the atomic
stock update in `ReservationRepository`, validate duplicate IDs before the
write, and retain the transaction and variant-cache invalidations in
`CreateReservation`. Register new reservation errors in both `ErrorCode` and
...
- `./gradlew.bat bootRun --args="--spring.profiles.active=dev"` — start the
  service on port 8090 with local infrastructure.
...
- `docker compose up -d` — start MySQL (host port 3306) and Redis (6379).
...
targets Java 25, so configure a compatible JDK before compiling. The `dev`
profile connects to MySQL database `inventory_database` with user
`inventory_user` and uses Redis on localhost.
...
shared exception handling layer. Reservation IDs must not be generated by JPA:
they are supplied by the caller. No formatter or linter is configured—match
...
`feat(category): add update endpoint`, `fix(product): reject duplicate SKU`,
or `feat(reservas): agregar reserva atómica`.
